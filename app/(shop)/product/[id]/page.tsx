...
export default function SingleProductView({ params }: { params: Promise<{ id: string }> }) {
    ...
    const onAddToCart = () => {
        ...
        // 2. Add to cart
        // (Note: To avoid double toasts, handle success/already-added toasts inside use-cart.ts OR remove toast calls from use-cart.ts)
        cart.addItem({
            id: product.id,
            name: product.name,
            price: product.price,
        ...
        <main className="max-w-7xl mx-auto px-6 py-12 lg:py-20 font-sans text-slate-900">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                ...
                <div className="lg:col-span-7 flex flex-row gap-4">
                    <div className="flex-1 aspect-[4/5] rounded-[2.5rem] overflow-hidden border border-slate-200 bg-white shadow-sm">
                        <img
                            src={selectedImage}
                ...
                <div className="lg:col-span-5 space-y-8">
                    <div>
                        <span className="bg-blue-50 text-blue-600 font-bold text-[10px] px-3 py-1 rounded-full uppercase tracking-widest border border-blue-100">
                        ...
                        <div className="flex gap-3">
                            {product.colors?.map((c: any) => (
                                <div key={c.id} className="group relative">
                                    ...
                                    />
                                    <span className="absolute -bottom-7 left-1/2 -translate-x-1/2 text-[9px] font-bold text-slate-500 whitespace-nowrap opacity-0 group-hover:opacity-100 transition-all">
                                        {c.name}
                                    </span>
                                </div>
                            ))}
