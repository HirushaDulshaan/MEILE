...
export default function MensProductsPage() {
    ...
    return (
        <main className="min-h-screen bg-[#F8FAFC] relative">
            <section className="max-w-[1600px] mx-auto px-6 py-12 lg:py-16">
                ...
                {/* MAIN GRID LAYOUT (3 Columns Sidebar + 9 Columns Products) */}
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">

                    {/* SIDEBAR FILTERS (Desktop Visible + Mobile Drawer) */}
                    <aside className={`
                        fixed inset-0 z-[100] bg-black/40 lg:bg-transparent lg:static lg:z-auto lg:col-span-3
                        transition-all duration-300
                        ${isDrawerOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none lg:opacity-100 lg:pointer-events-auto"}
                    `}>
                        <div className={`
                            fixed left-0 top-0 h-full w-80 bg-white p-6 shadow-2xl overflow-y-auto lg:overflow-visible lg:p-0 lg:w-full lg:h-auto lg:bg-transparent lg:static lg:shadow-none
                        ...
                        `}>
                            ...
                            {/* Category Filter */}
                            <div className="bg-white rounded-3xl border border-slate-100 overflow-hidden mb-4 shadow-sm">
                                ...
                                {openFilters.category && (
                                    <div className="px-4 pb-5 flex flex-col gap-1.5 animate-in fade-in slide-in-from-top-2">
                                        <button onClick={() => {setSelectedCategory("all"); setIsDrawerOpen(false);}} className={`text-[12px] font-bold text-left px-4 py-2.5 rounded-xl transition-all ${selectedCategory === "all" ? 'bg-blue-600 text-white shadow-md' : 'text-slate-600 hover:bg-slate-50'}`}>All Collections</button>
                                        {categories.map((cat) => (
                                            <button key={cat.id} onClick={() => {setSelectedCategory(cat.name); setIsDrawerOpen(false);}} className={`text-[12px] font-bold text-left px-4 py-2.5 rounded-xl transition-all ${selectedCategory === cat.name ? 'bg-blue-600 text-white shadow-md' : 'text-slate-600 hover:bg-slate-50'}`}>{cat.name}</button>
                                        ))}
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="bg-white rounded-3xl border border-slate-100 overflow-hidden mb-4 shadow-sm">
                                ...
                                {openFilters.size && (
                                    <div className="px-4 pb-5 flex flex-wrap gap-2 animate-in fade-in slide-in-from-top-2">
                                        <button onClick={() => {setSelectedSize("all"); setIsDrawerOpen(false);}} className={`px-3 h-8 rounded-lg text-[9px] font-black border-2 ${selectedSize === "all" ? 'bg-slate-900 border-slate-900 text-white' : 'border-slate-100 text-slate-400'}`}>ANY</button>
                                        {sizes.map((s) => (
                                            <button key={s.id} onClick={() => {setSelectedSize(s.sizeCode); setIsDrawerOpen(false);}} className={`px-3 h-8 rounded-lg text-[9px] font-black border-2 transition-all ${selectedSize === s.sizeCode ? 'bg-slate-900 border-slate-900 text-white shadow-md' : 'border-slate-100 text-slate-400 hover:border-blue-600'}`}>{s.sizeCode}</button>
                                        ))}
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="bg-white rounded-3xl border border-slate-100 overflow-hidden mb-4 shadow-sm">
                                ...
                                {openFilters.color && (
                                    <div className="px-4 pb-5 flex flex-wrap gap-3 animate-in fade-in slide-in-from-top-2">
                                        <button onClick={() => {setSelectedColor("all"); setIsDrawerOpen(false);}} className={`w-7 h-7 rounded-full border-2 border-slate-200 bg-slate-100 ${selectedColor === "all" ? 'ring-2 ring-blue-600 ring-offset-2' : ''}`} title="All Colors" />
                                        {colors.map((c) => (
                                            <button key={c.id} onClick={() => {setSelectedColor(c.name); setIsDrawerOpen(false);}} className={`w-7 h-7 rounded-full border border-slate-100 transition-all ${selectedColor === c.name ? 'ring-2 ring-blue-600 ring-offset-2 scale-110 shadow-md' : 'hover:scale-105'}`} style={{ backgroundColor: c.hexCode }} title={c.name} />
                                        ))}
                                    </div>
                                )}
                            </div>

                            <button onClick={() => setIsDrawerOpen(false)} className="lg:hidden w-full mt-6 bg-blue-600 text-white py-4 rounded-2xl font-black uppercase tracking-widest text-xs">Show Results</button>
                        </div>
                    </aside>
                            ...
                            <>
                                {sortedProducts.length > 0 ? (
                                    ...
                                    <div className="bg-white rounded-[2.5rem] p-20 text-center border border-dashed border-slate-200">
                                        <p className="text-slate-400 font-bold uppercase tracking-widest text-xs">No products found matching filters.</p>
                                    </div>
                                )}
                            </>
                        )}
